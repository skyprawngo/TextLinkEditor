...
final class ManuscriptInlinePanelController {
    ...
    func textDidChange() {
        ...
        anchor = min(max(0, anchor), editor.textStorage?.length ?? 0)
        invalidateLayout()
    }
    ...
    func makeFragment(for element: NSTextElement, manager: NSTextLayoutManager) -> NSTextLayoutFragment {
        ...
        if panel != nil, let content = manager.textContentManager, let range = element.elementRange {
    ...
    func layout() {
        guard let editor, let panel, let location = editor.textLocation(at: anchor),
