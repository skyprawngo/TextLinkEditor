...
- `CLI/CLIProcessManager.swift`: Claude stream-json / Codex exec JSONL의 명시적 완료 이벤트와 종료 코드를 처리한다. 추가 지시를 허용한 Codex 사이드바 채팅은 `Requests/CodexActiveTurn`의 전용 App Server 연결로 실행한다. 각 ViewModel은 자체 실행기를 가지며, 요청별 POSIX 프로세스 그룹을 취소한다. 지속 터미널 모드는 지원하지 않는다.
- `Prompt/AIPromptTemplateManager.swift`와 `Resources/AIPromptTemplates.json`: 문맥 조립만 담당한다. CLI 인자와 출력 계약은 CLIProcessManager에 둔다.

## 응답 중 입력과 대기열

`Requests/AIChatSubmissionQueue`가 대기 항목, 일시정지, 추가 지시 수락 대기의 단일 소유자다. ViewModel은 초안 스냅샷·실행·기록 저장을 조율하고 성공/실패를 큐에 알린다. 추가 지시 전송 시도마다 별도 토큰을 사용하므로 취소 전 콜백이 같은 항목의 재시도를 완료할 수 없다. `AIQueueReordering`은 포인터 진입과 애니메이션만 처리하며 순서 변경은 큐에 위임한다.

사이드바 입력은 응답 중에도 편집 가능하다. Enter는 `AIQueuedMessage`에 본문·대화 ID·제공자·모델·모드·첨부 스냅샷을 접수하며, 응답 완료와 기록 저장 후 FIFO로 다음 요청을 실행한다. `Chat/AIMessageQueueView`는 입력창 위의 대기열, 드래그 중 항목 진입마다 슬라이드 애니메이션으로 순서 변경(동작 줄이기 설정 존중), 삭제, 회수, 현재 응답에 추가 지시 전송을 제공한다. 회수할 때 작성 중 초안은 같은 대기 위치로 교환한다. 실패나 중단은 자동 진행을 멈추며 대기 항목은 남긴다. 대기열은 실행 중 메모리 상태로, 프로젝트 전환이나 기록 초기화 때 비운다.

⌘Enter는 대기열을 거치지 않고 같은 대화의 활성 응답에 추가 지시를 보낸다. Codex `turn/steer`에 thread ID와 expected turn ID를 함께 전달하며 수락 응답을 받은 뒤에만 입력/대기 항목을 소비한다. 완료 이벤트가 수락 응답보다 먼저 도착해도 확인할 때까지 연결을 유지한다. 거절·준비 중·이미 종료·다른 대화이면 입력을 보존하고 오류를 표시한다. Claude 공개 CLI는 같은 턴에 추가 지시를 보장하지 않으므로 이를 지원하지 않는다고 표시하며 Enter 대기열은 사용할 수 있다. 인라인 편집과 커밋 메시지 생성에는 추가 지시를 섞지 않는다.

추가 지시는 새 assistant 응답을 만들지 않고 해당 응답 앞의 user 메시지로 기록한다. 다음 문맥에는 원래 지시와 추가 지시를 함께 포함한다. 초안의 수정 세대로 소유권을 확인하여 실행 실패나 수락 콜백이 새 입력을 덮어쓰지 않는다. 한글 조합 확정은 전송하지 않으며 Shift+Enter는 줄바꿈이다.

## 채팅 입력 초안

`Chat/AIChatDraftStore`는 전송하지 않은 본문을 로컬 UserDefaults에 프로젝트의 정규화된 경로와 conversation ID로 캐싱한다. 새 채팅은 프로젝트별 `new` 키를 사용한다. 대화/프로젝트 전환과 패널 재생성 후 복원하며 앱 재실행에도 유지한다. 프로젝트를 Finder에서 다른 경로로 이동한 경우의 초안 이전은 지원하지 않는다. 공백·줄바꿈은 그대로 보존하고 빈 입력은 캐시에서 제거한다. 전송 기록이나 외부 제공자 세션에 초안을 포함하지 않는다.

입력 변경마다 캐시를 갱신하며 UserDefaults의 비동기 디스크 반영을 사용한다. 전송 접수·대기열 접수 때 해당 초안을 비우고 대화 삭제/기록 초기화가 성공하면 해당 범위의 초안을 제거한다. 회수할 대기 항목이 다른 대화에 속하면 원래 대화 초안은 캐시에 남기고 대상 대화 초안을 대기열과 교환한다. 입력 binding과 네이티브 입력 뷰는 프로젝트/대화에 귀속되어 늦은 입력 이벤트와 Undo가 다른 대화에 섞이지 않는다. 캐싱 대상은 본문이며 첨부·모드·커서 위치는 영속 초안에 포함하지 않는다.

## 저장 형식

일반 채팅은 프로젝트 루트의 텍스트 문서를 격리 사본에 복사해 작업 디렉토리로 사용하고, 프로젝트를 다시 열면 현재 제공자의 최근 일반 대화를 기본으로 선택한다. 후속 요청은 대화별 저장된 제공자 세션 ID로 재개한다(Codex `thread/resume`, Claude `--resume`). 매번 현재 사본의 경로와 최신 문서를 우선하도록 안내한다. 새 대화는 새 세션이며 인라인 편집은 항상 독립 요청이다. 세션은 마지막 완료 응답의 제공자가 일치할 때만 사용한다. 실패·취소 후에는 세션 연결을 해제하고 사용자가 재시도하면 저장된 완료 대화로 문맥을 구성한다. 앱 기록은 프로젝트에, 제공자의 원본 세션은 해당 CLI 저장소에 있으므로 프로젝트 복사만으로 외부 세션까지 이동되지는 않는다.
...
`CollaborationApplier`만 실제 텍스트를 수정한다. 경로·보호 파일·열린 초안·IME를 검증하고 텍스트 변경을 최신 원고에 병합하며, 다중 파일 변경 전 `.{프로젝트명}.weavedata/collaboration`에 복구 저널을 저장한다. 중간 종료는 재진입 시 복구하며 후속 사용자 변경이 있으면 중단한다. 되돌리기도 같은 충돌 검사를 통과해야 한다. 작업 큐·설정 버전·기준본은 `state.json`, 변경 전후는 트랜잭션별 저널에 저장한다. 이미지·숨김 경로·심볼릭 링크·앱 데이터·Git 관리 파일은 사본과 수정 대상에서 제외한다. 패널을 닫아도 작업은 유지하며 프로젝트 전환은 취소한다. 협업 메타데이터와 일반 대화 기록의 생명주기는 별도다.

인라인 편집(`Views/MainEditor/AIAssistant/Inline/InlineAIChatView.swift`)은 전송 직후 닫히는 단일 지시 입력창이다. 선택한 범위가 있으면 치환하고 공백 선택 또는 커서만 있으면 해당 위치에 삽입한다. 문맥은 화면에 표시하지 않는다. `InlineEditRequest`가 JSON replacement 응답을 요구하며 앱만 기준본 비교·버전 기록·네이티브 Undo 경로를 통해 수정한다. 활성 문서가 바뀌면 적용하지 않으며 원고가 바뀌면 텍스트 병합으로 독립적인 변경을 보존한다. 일반 답변이나 잘못된 JSON은 실패로 기록한다.
...
- `CLI/CodexRPCConnection`: 계정 서비스와 활성 턴 실행기가 각각 생성하는 공통 JSONL RPC 연결. 프로세스·요청 ID·응답/알림·종료만 소유하며 계정 상태나 대기열을 변경하지 않는다.
...
기존 원고의 제안은 `CollaborationEdit.replacements`의 `oldText`/`newText`로 지정한다. 줄 번호를 받지 않는다. 기준본에서 한 번만 나타나는 정확한 원문을 요구하며 삽입은 문맥을 포함한 교체, 구간 삭제는 빈 `newText`로 표현한다. 새 파일/파일 삭제와 구형 제안은 `content` 계약을 유지한다.

`Workspace/DocumentReconciliation.swift`의 `ManuscriptTextMerge`는 기준본·현재본·제안본으로 텍스트 패치를 산출한다. AI 적용 전 IME 조합 여부를 확인하고 열린 초안을 저장한 뒤 최신 디스크에 패치를 재배치한다. 서로 다른 변경은 보존하며 AI가 완전히 삭제하는 구간 안의 변경은 삭제에 포함한다. 같은 위치의 다른 삽입, 서로 다른 겹친 교체, 삭제 구간 경계를 넘어선 변경은 충돌이다. 충돌 시 해당 적용 배치를 쓰기 전에 중단하고 제안을 review 상태로 남긴다. 파일 생성/삭제는 여전히 존재 여부와 전체 기준본이 일치해야 한다.

저널에는 AI 요청 시점이 아닌 실제 병합 전후 본문을 기록한다. Undo는 그 차이를 최신 본문에 역병합하여 이후의 독립적인 수정도 유지한다. 병합 후 실제 원고에 남아 있는 인용만 설정 사실로 등록한다. 일반 저장·외부 갱신은 같은 병합기를 쓰되 삭제 우선권을 부여하지 않는다. 인라인 제안도 캡처한 선택을 기준본에서 먼저 치환한 다음 최신 편집기에 병합하고 네이티브 Undo 경로로 적용한다. 차이 계산의 상한을 넘으면 자동 덮어쓰기로 대체하지 않고 보류한다.
